...
// /!\ Execute this script line by line to switch from statement to expression (an expression return a value displayed into the console)
...
//So if I remove the ';' at the end it is a kind of expression in csx and the value is not displayed but multiple line fail at compile/design time because the ';' at the end is missing ?!
...
//Partial application can replace depency injection frameworks (dependency injection with partial application) and could reduce complexity in framework (WCF, Kestrel and so on..)
